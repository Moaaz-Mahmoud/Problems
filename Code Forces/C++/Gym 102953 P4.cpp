...
#define MAX 100009
ll n, m, k, a, b, c;
bool isMagic(const vi& A, int sz){
	bool can = 1;
	for(int i = 0; i < sz/2; i++){
		if(A[i] != A[i + sz/2]) can = 0;
	}
	return can;
}
void init(){
...
void solve(){
	...
	int c; cin >> c;
	set<int> Leaders, Sick;
	loop(c){
		int a; cin >> a;
		Leaders.insert(D.findLeader(a));
		Sick.insert(a);
	}
	set<int> Add;
	for(int i = 1; i <= n; i++){
		if(Leaders.find(D.findLeader(i)) != Leaders.end() &&
		   Sick.find(i) == Sick.end())
			Add.insert(i);
	}
	cout << Add.size() << '\n';
	for(auto a : Add) cout << a << ' ';
}
