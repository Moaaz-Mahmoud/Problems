...
#define double long double
	while (t--) {
		int n, l; cin >> n >> l;
		vi A(n); loop(n) cin >> A[i];
		double a = 0, b = l, sa = 1, sb = 1, time = 0;
		int i = 0, j = n - 1;
		for (; j - i > 0;) {
			if ((A[i] - a)/sa < (b - A[j])/sb) {
				double delta_t = (A[i] - a) / sa;
				time += delta_t;
				a = A[i];
				sa++;
				b -= delta_t * sb;
		...
		lab:;
			double left = 0, right = b - a, mid;
			while (right - left >= 1e-6) {
				mid = (left + right) / 2;
				if (a + sa * mid > b - sb * mid) right = mid;
				else                             left = mid;
			}
