...
int main()
{
	IOS;
	int n; cin >> n;
	int largestSmaller = -1, largestLarger = -1;
	while (n--) {
		char q;
		int smaller, larger;
		cin >> q >> smaller >> larger;
		if (smaller > larger) swap(smaller, larger);
 
		if (q == '+') {
			largestSmaller = max(largestSmaller, smaller);
			largestLarger = max(largestLarger, larger);
		}
		else {
			if (smaller < largestSmaller || larger < largestLarger)
				cout << "NO\n";
			else
				cout << "YES\n";
		}
