...
int main()
{
	IOS;
	int n; cin >> n;
	vii A(n); loop(n) cin >> A[i].first >> A[i].second;
	int ans = n > 1 ? 2 : 1,
		rightMostOcc = A[0].first;
	for (int i = 1; i + 1 < n; i++) {
		if (A[i].first - A[i].second > rightMostOcc) {
			ans++;
			rightMostOcc = A[i].first;
		}
		else {
			if (A[i].first + A[i].second < A[i + 1].first) {
				ans++;
				rightMostOcc = A[i].first + A[i].second;
			}
			else {
				rightMostOcc = A[i].first;
			}
