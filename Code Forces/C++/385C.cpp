...
int main()
{
	...
	while (_t_--) {
		...
		vi FD(10000001);	//FD[i] = Frequency of multiples of A[i] in A IF A[i] is prime.
		for (int p : P) {
			for (int j = p; j < 10000001; j += p)
				FD[p] += F[j];
		}
		for (int i = 1; i < FD.size(); i++) FD[i] += FD[i - 1];
 
		int m; cin >> m;
		loop(m) {
			int l, r;
			cin >> l >> r; 
			l = min(l, 10000000); r = min(r, 10000000);
			cout << FD[r] - FD[l - 1] << '\n';
		}
	}
