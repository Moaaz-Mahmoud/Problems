...
void solve(){
	...
	vi B;
	int ans=0;
	for(auto a : A) F[a%10] = min(F[a%10]+1, 3);
	loop(F.size()) while(F[i]--) B.push_back(i);
	for(int i = 0; i < B.size(); i++){
		for(int j = 0; j < B.size(); j++)if(j!=i){
			for(int k = 0; k < B.size(); k++)if(k!=i&&k!=j){
				if((B[i]+B[j]+B[k])%10==3) ans=1;
			}
		}
	}
	cout << (ans?"YES":"NO") << '\n';
