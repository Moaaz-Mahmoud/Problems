...
struct submission{
	int time, id, problem, stat;
	bool operator < (submission right){
...
struct contestant{
	...
	set<int> solved;
	bool operator < (contestant right){
		if(problems > right.problems) 
			return 1;
		else if(problems < right.problems)
			return 0;
		else{
			if(penalty < right.penalty)
				return 1;
	...
	friend ostream& operator << (ostream& s, contestant& c){
		s << c.id << ' ' << c.problems << ' ' << c.penalty;
	}
};
...
int main()
{
	...
	while (_t_--) {
		int c, p, s;
		cin >> c >> p >> s;
		vector<contestant> A(c); loop(c) A[i].id = i+1;
		vector<submission> B(s);
		loop(s){
			int time, id, problem, stat;
			cin >> time >> id >> problem >> stat;
			id--;
			B[i] = {time, id, problem, stat};
			if(stat == 1) {
				A[id].solved.insert(problem);
			}
		}
		for(contestant &a : A) a.problems = a.solved.size();
		sort(all(B));
		for(submission b : B){
			if(A[b.id].solved.find(b.problem) != A[b.id].solved.end())
				A[b.id].penalty += b.time;
			if(b.stat == 1) A[b.id].solved.erase(b.problem);
		}
		sort(all(A));
		for(contestant a : A)
			cout << a << '\n';
