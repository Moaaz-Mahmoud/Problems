...
// last_a: friend = 1, me = 2
ll sol(ll p, ll last_a){
	return 1;
}

ll a, b, l, r;

bool validate(ll bit){
	ll n = (1 << bit);

	for(ll i = 32; i >= 0; i--){
		if(i == bit) continue;
		ll after_set = (1LL<<i) + n;
		if(after_set > r){
			continue;
		}
		else if(l <= after_set && after_set <= r){
			return true;
		}
		else{
			n += (1LL << i);
...
void solve(){
	...
	bool ans = 1;
	for(ll i = 0; i < to_set.size(); i++){
		ans = ans && validate(to_set[i]);
	}
	if(!ans){
		cout << "NO\n";
		return;
